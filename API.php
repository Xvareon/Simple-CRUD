...
class API {
     ...
     * @param $payload
     */
    public function httpGet($id, $payload = array()){
 
        // Construct the WHERE clause of the SQL query based on the $id parameter
        if(is_array($id) && count($id) > 0){
            $where_clause = 'id IN (' . implode(',', $id) . ')';
        } else if(!empty($id)){
            $where_clause = 'id = ' . $id;
        } else {
            $where_clause = '1=1'; // To fetch all records from table
        }
